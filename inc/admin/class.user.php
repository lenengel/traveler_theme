    ...
    class STUser extends STAdmin
    {
        ...
        public function st_notifyauthor_callback( $post){
            ...
            if (!empty( $author->roles ) && in_array( 'partner' , $author->roles ) && st()->get_option('partner_post_by_admin', 'on') == 'on') {
                ...
                $message .= sprintf(__( "Your service item, %s has just been published.", 'traveler' ),$post->post_title);
                $message .= '<br>';
                $message .= __( 'View this service item ', 'traveler' ) .' <a href="'.get_permalink( $post->ID ).'">'.$post->post_title.'</a>';
	    ...
	    public static function verify_status($user_id){
			...
			$list_key =	st_get_user_verify_keys();
			$count_c  = count( $list_key );
		...
		static function _send_customer_register_normal($user_id)
        {
            global $st_user_id;
            $st_user_id = $user_id;
            $user_data = get_userdata($user_id);
            ...
            $to = $user_email;
            $message          = "";
            $register_email_for_customer = st()->get_option('register_email_for_customer', '');
            $content = get_post($register_email_for_customer);
            $message .=  do_shortcode($content->post_content);
            $title = get_option('blogname');
			$subject = sprintf(__('Your account at %s is created!', 'traveler'), $title);
        ...
        private static function _send_mail_user($to, $subject, $message, $attachment = false)
        {
            ...
            $from_address = st()->get_option('email_admin_address');
            $headers = [];
